...
class CustomBengioDataset(torch.utils.data.Dataset):
    def __init__(self, corpus: list[int], rev_words:dict[str:int], window: int, verbose=False, cuda=False,**kwargs):
        self.window = window
        self.corpus = corpus
    ...
    def __getitem__(self, idx):
        ...
        if self.cuda:
            word_id_list_tensor = torch.tensor(raw[:-1], dtype=torch.long).cuda()
            next_word_id_tensor = torch.tensor(raw[-1], dtype=torch.long).cuda()
        else:
