...
class PositionalEncoder(nn.Module):
    def __init__(self, d_model, max_seq_len = 4096, dropout = 0.1):
        super().__init__()
        self.d_model = d_model
        #takes in a tensor and outputs the same tensor shape but zeroes out random values
        ...
        pe = pe.unsqueeze(0)
        #remembers the positional encodings for the model so that it only has to be run one time at the start
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, e_outputs, src_mask, trg_mask):
        ...
        x2 = self.norm_2(x)
        x = self.dropout_2(self.attn_2(x2, e_outputs, e_outputs, \
        src_mask))
        x2 = self.norm_3(x)
...
class Transformer(nn.Module):
    def __init__(self, src_vocab:int, trg_vocab:int, d_model:int, N:int, heads:int, dropout:float|int):
        super().__init__()
